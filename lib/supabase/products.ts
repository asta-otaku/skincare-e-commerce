...
export async function getFeaturedProducts(limit = 8): Promise<Product[]> {
  const supabase = getReadClient()
  if (!supabase) {
    return filterMock({ sort: "rating", limit }).filter(
      p => p.tag === "New" || p.tag === "Bestseller",
    ).slice(0, limit).concat(
      filterMock({ sort: "rating", limit }),
    ).filter((p, i, arr) => arr.findIndex(x => x.id === p.id) === i).slice(0, limit)
  }
  ...
  const taggedList = (tagged ?? []).map(rowToProduct)
  if (taggedList.length >= limit) return taggedList

  const more = await queryProducts({ sort: "rating", limit: limit * 2 })
  const seen = new Set(taggedList.map(p => p.id))
  const filled = [...taggedList]
  for (const p of more) {
