...
import { getBrandStorefrontSummaries } from "@/lib/supabase/products"
      ...
      {brandsWithStock.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {brandsWithStock.map(brand => (
            <div key={brand.id} id={brand.id} className="group border border-border p-6 transition-all hover:border-gold/60 hover:bg-secondary">
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-1">
                  {brand.sampleNames.map(name => (
                    <li key={name} className="flex items-center gap-1.5 text-xs font-light text-muted-foreground">
